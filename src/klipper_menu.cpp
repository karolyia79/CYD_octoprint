...
#include "ui_utils.h"
#include "config_manager.h" // Szükséges a getCurrentTheme()-hez
...
void KlipperMenu::drawMainMenu() {
    ...
    UIUtils::drawButton(_tft, 20, 80, 280, 38, LangManager::get("klipper_btn_macros"), theme.cardBg, theme.text, false, 2, 5);

    // 2. Gomb: Hangolás (Tune / Z-offset)
    UIUtils::drawButton(_tft, 20, 128, 280, 38, LangManager::get("klipper_btn_tune"), theme.cardBg, theme.text, false, 2, 5);

    // Vissza gomb
    UIUtils::drawButton(_tft, 20, 185, 280, 38, LangManager::get("btn_back"), theme.cardBg, theme.text, false, 2, 5);
}
...
void KlipperMenu::drawTuneMenu(KlipperClient* client) {
    ...
    _tft->setTextColor(theme.accent, theme.bg);
    _tft->setTextDatum(TC_DATUM);
    _tft->drawString(LangManager::get("klipper_tune_title_screen"), 160, 48, 2);
    ...
    drawTuneRow(72,  LangManager::get("klipper_label_speed"),  String(spd) + "%");
    drawTuneRow(106, LangManager::get("klipper_label_nozzle"), String(nzl) + "C");
    drawTuneRow(140, LangManager::get("klipper_label_bed"),    String(bed) + "C");
    ...
    _tft->setTextColor(theme.text, theme.cardBg);
    _tft->setTextDatum(MC_DATUM);
    _tft->drawString(LangManager::get("klipper_label_zoffset"), 160, 189, 2);
...
int KlipperMenu::handleTouch(uint16_t x, uint16_t y, KlipperClient* client) {
    ...
    if (_subState == 0) {
        if (y >= 185 && y <= 223 && x >= 20 && x <= 300) {
            UIUtils::pressFeedback(_tft, 20, 185, 280, 38, LangManager::get("btn_back"), theme.cardBg, theme.text, 2, 5);
            return 0; // Vissza a főképernyőre
        }
        if (y >= 80 && y <= 118 && x >= 20 && x <= 300) {
            UIUtils::pressFeedback(_tft, 20, 80, 280, 38, LangManager::get("klipper_btn_macros"), theme.cardBg, theme.text, 2, 5);
            return 30; // Makrók indítása
        }
        if (y >= 128 && y <= 166 && x >= 20 && x <= 300) {
            UIUtils::pressFeedback(_tft, 20, 128, 280, 38, LangManager::get("klipper_btn_tune"), theme.cardBg, theme.text, 2, 5);
